...
입력(Excel/JSON/이미지/PDF/drawio)과 출력(PNG/JSON/HTML/Excel/drawio) 포맷을 자유롭게 조합할 수 있다.
...
- [x] 실제 exe를 스크린샷으로 직접 클릭해보는 실사용 테스트 — 메인윈도우/파일 메뉴/
      내보내기 하위메뉴까지 시각적으로 검증 완료
- [x] PDF/JSON은 더미 데이터로 검증 완료, 이걸로 충분하다고 판단 — Excel/PDF/JSON이
      전부 같은 `_call_llm_text` 경로를 타므로 Excel의 실제 파일 검증(75/75 완전성)이
      그 경로 전체의 신뢰성을 사실상 같이 보증함
- [x] **Undo/Redo 추가** — "편집" 메뉴(Ctrl+Z/Ctrl+Shift+Z). 행 추가/삭제/색상변경/
      텍스트·날짜 편집 시작 직전에 전체 상태를 스냅샷(`copy.deepcopy`)으로 쌓는 방식.
      세밀한 Command 객체 대신 단순 스냅샷을 쓴 이유: WBSData 트리가 작아서(보통
      task 수백 개 이내) 매번 깊은 복사해도 비용이 작고, 구현이 훨씬 단순해 버그 위험이 적음
- [ ] 다중 도파/탭 (여러 WBS 파일을 동시에 열고 교차) — 보류 중, 필요해지면 진행
- [ ] "보기" 메뉴 (확대/축소, 그리드 표시 옵션 등) — 보류 중, 필요해지면 진행
- [x] **"새로 만들기"에 기본 템플릿 추가** — 파트1/소분류1/작업1(오늘~+7일)이 자동으로
      만들어져서 빈 상태가 아니라 바로 수정할 수 있는 시작점이 생김
- [x] **미리보기 차트에서 직접 마우스로 편집** (`gui.PreviewLabel`, `renderer.x_to_date`/
      `row_hit_areas`) — 소분류 행 영역 드래그로 새 작업 추가, 작업 막대 안쪽 드래그로
      이동, 막대 좌/우 끝(6px 이내) 드래그로 시작일/종료일만 개별 조정. 모두 Undo 지원.
      실시간 드래그 미리보기(고무줄 표시)는 1차 버전에서는 생략 — 누른 지점/뗀 지점만으로 판단
- [x] "설정" 메뉴(API 키 변경) / "도움말" 메뉴(사용법/정보) 추가, 툴바 완전히 제거
      (모든 기능이 메뉴로 정리됨)
- [x] 셀 직접 편집(텍스트/날짜) 시 미리보기 자동 갱신 — `itemChanged` 시그널에 연결,
      `_populating` 플래그로 프로그램에 의한 테이블 채움과 사용자 편집을 구분.
      구조 변경(추가/삭제/색상)은 원래도 자동 갱신이었어서, 이제 수동 새로고침 버튼 자체가
      불필요해져 제거함
